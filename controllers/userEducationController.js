...
module.exports = {
    get:(req,res)=>{
        ...
        User.getUserId(username).then((user)=>{
            ...
            User.getUserEducation(user._id).then((data)=>{
                var mp = new MapPaths(req.protocol+"://"+ req.get('host'));
                data.map((value)=>{
                    value.logo = mp.getEducationImage(username,value.logo);
                    return value;  
                });
        ...
        if(!data){
            ...
            User.getUserId(username).then((user)=>{
                if(!user){
                    ...
                    UserData.editUserEducation(user._id.toString(),data).then((newUser)=>{
                        if(newUser){
        ...
        if(!data){
            ...
            User.getUserId(username).then((user)=>{
                if(!user){
                    ...
                    UserData.UpdateEducationOrder(data).then((newUser)=>{
                        if(newUser){
                            var responseData = new Response({Status:200,Data:newUser});
                            res.send(responseData.getResponse());
                        }
        ...
        if(!data){
            ...
            User.getUserId(username).then((user)=>{
                if(!user){
                    ...
                    UserData.addUserEducation(user._id.toString(),data).then((newUser)=>{
                        if(newUser){
                            var responseData = new Response({Status:200,Data:newUser});
                            res.send(responseData.getResponse());
                        }
        ...
        if(!data){
            ...
            User.getUserId(username).then((user)=>{
                if(!user){
                    ...
                    UserData.deleteUserEducation(user._id.toString(),data).then((newUser)=>{
                        if(newUser){
